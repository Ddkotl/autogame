...
export async function atackWerwolfs(session_id: string) {
  try {
    const pageUrl = "https://mvoo.ru/clan/castle/?werewolves=true";
    const html = await curlGet(pageUrl, session_id);

    if (!html) {
      console.warn("Не удалось получить страницу оборотней");
      return;
    }
    ...
    const attackUrl = attackHref.startsWith("http") ? attackHref : `https://mvoo.ru${attackHref}`;
