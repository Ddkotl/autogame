...
(async () => {
  ...
  fs.writeFileSync(
    "./sacrifice_list.json",
    JSON.stringify(sacrifice_list, null, 2),
    "utf-8",
  );
  console.log(
    "sacrifice list saved",
    sacrifice_list.length,
  );
})();

async function getSacrificeId(lvl: number) {
  const res = await fetch(
    `https://mvoo.ru/arena/main/?sorting=${lvl}&last=true`,
    {
      headers: {
